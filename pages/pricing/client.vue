...
// Frontend display model (what PricingPlanCard expects)
type PlanDisplay = {
  name: string
  tier: Tier
  price: string
...
// Internal model used for selection + checkout (includes planId)
interface Plan extends PlanDisplay {
  planId: string // maps to your DB/Stripe price
}
...
function mockFetchPlans(): Promise<Plan[]> {
  // These map to your server-side plan ids (e.g., plans.plan_id)
  const data: Plan[] = [
    {
      planId: 'client_free',
      name: 'Free',
...
function mockGetCurrentPlan(): Promise<{ planId: string }> {
  // Pretend the user currently has the free plan
  return new Promise((resolve) => setTimeout(() => resolve({ planId: 'client_free' }), 200))
}

function mockCreateCheckoutSession(planId: string): Promise<{ url: string }> {
  // Simulate creating a Stripe Checkout Session and returning a redirect URL
  return new Promise((resolve) => setTimeout(() => resolve({ url: `/billing/monthck-checkout?plan=${planId}` }), 400))
}
...
// Label for the primary action based on selection
const primaryCta = computed(() => {
  if (!selectedPlan.value) return 'Continue'
  if (isSelectingCurrent.value) return 'Current plan'
  // Simple copy; customize if you want per-tier wording
...
async function loadPage() {
  ...
  try {
    ...
    // Default selection: current plan (or first plan)
...
async function proceed() {
  ...
  try {
    ...
    // For a real app, do: window.location.href = url
        ...
        <div
          ...
          <div class="max-w-7xl mx-auto px-4 py-4 flex flex-col md:flex-row items-center justify-between gap-3">
            ...
            <div class="flex items-center gap-3 w-full md:w-auto">
              <!-- <button type="button"
                class="px-4 py-2 rounded border border-black/10 dark:border-white/10 text-black dark:text-white hover:bg-black/5 dark:hover:bg-white/10 transition w-full md:w-auto"
                @click="loadPage" :disabled="loading || creating">
                Refresh
              </button> -->
